...
// constructor
var AnyBoidy = function(posx, posy, settings) {
	...
	this.stackFSM.pushState(this.flockWander, this);
};


AnyBoidy.prototype = {
	constructor: AnyBoidy,
	...
	fleeMouse: function fleeMouse() {
		Steering.flee(this.boid, Kai.mouse.position);
		
		if (this.position.distanceTo(Kai.mouse.position) > 250) {
			// safe now, remove this state and go back to whatever we were doing before
			this.stackFSM.popState();
		}
	},
	
	flockWander: function flockWander() {
		this.flock();
		Steering.wander(this.boid);
		
		if (this.position.distanceTo(Kai.mouse.position) < 200) {
			// too close, start running away chaotically (no flocking) on next tick
			this.stackFSM.pushState(this.fleeMouse, this);
		}
	},
	...
	_stateChanged: function(oldState, newState) {
		// console.log('[AnyBoidy._stateChanged] from '+oldState+' to '+newState);
	}
